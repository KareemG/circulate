...
passport.use(new LocalStrategy({ usernameField: 'email' }, function(email, password, done) {
  Employee.findOne({ email: email.toLowerCase() }, function(err, user) {
    if (!user) {
...
passport.use(new LinkedInStrategy({
  ...
  if (req.user) {
    Employee.findOne({ linkedin: profile.id }, function(err, existingUser) {
      if (existingUser) {
    ...
    Employee.findOne({ linkedin: profile.id }, function(err, existingUser) {
      ...
      Employee.findOne({ email: profile._json.emailAddress }, function(err, existingEmailUser) {
        if (existingEmailUser) {
